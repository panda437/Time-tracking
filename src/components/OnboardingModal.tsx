...
export default function OnboardingModal({ isOpen, onClose, onSaveGoals }: OnboardingModalProps) {
  ...
  const handleSaveGoals = async () => {
    ...
    try {
      ...
      const goals = selectedGoals.map(goal => ({
        ...
        goalType: "other",
        milestones: [],
        isRefined: false // Mark as not refined so users can refine later
      }))
      ...
      if (response.ok) {
        setSavedGoals(goals)
        setShowCategoryModal(true) // Show category selection after goals are saved
      } else {
  ...
  const handleCategorySave = (categories: string[]) => {
    // Goals are already saved, categories are saved by CategorySelectionModal
        ...
        <div className="bg-white rounded-2xl max-w-lg w-full max-h-[96vh] overflow-y-auto shadow-xl">
          <div className="p-4 sm:p-6 text-sm sm:text-base">
            ...
            <div className="flex justify-end space-x-3 sticky bottom-0 bg-white pt-4 pb-2">
              ...
              >
                {isSaving ? 'Saving...' : 'Next: Choose Categories'}
              </button>
            </div>
          </div>
        </div>
